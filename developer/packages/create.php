...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($slug === '' || !preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $slug) || mb_strlen($slug) > 120) {
        $errors[] = 'Slug wajib berisi huruf kecil, angka, dan strip.';
    }
    ...
    if (!$errors) {
        $check = $pdo->prepare('SELECT id FROM packages WHERE slug = :slug LIMIT 1');
...
<div class="p-4 md:p-8 max-w-3xl mx-auto">
    ...
    <h1 class="text-2xl md:text-3xl font-semibold tracking-tight mt-1">Tambah Paket</h1>
    <p class="text-sm text-neutral-500 mt-2 mb-6">Paket baru langsung berstatus ACTIVE.</p>
    ...
    <form method="post" class="bento-card p-5 md:p-7 space-y-5">
        ...
        <div>
            ...
            <input id="name" name="name" required maxlength="100" value="<?= e($name) ?>" class="w-full h-11 rounded-xl border border-neutral-200 px-4 text-sm outline-none focus:border-neutral-400" placeholder="Contoh: Basic">
        </div>
        <div>
            <label class="block text-sm font-medium mb-2" for="slug">Slug</label>
            <input id="slug" name="slug" required maxlength="120" value="<?= e($slug) ?>" class="w-full h-11 rounded-xl border border-neutral-200 px-4 text-sm outline-none focus:border-neutral-400" placeholder="basic">
            <p class="text-xs text-neutral-400 mt-2">Huruf kecil, angka, dan strip. Harus unik.</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-medium mb-2" for="price">Harga</label>
                <input id="price" name="price" type="number" min="0" step="0.01" required value="<?= e($price) ?>" class="w-full h-11 rounded-xl border border-neutral-200 px-4 text-sm outline-none focus:border-neutral-400" placeholder="99000">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2" for="duration_days">Durasi</label>
                <div class="relative">
                    <input id="duration_days" name="duration_days" type="number" min="1" max="36500" required value="<?= e($durationDays) ?>" class="w-full h-11 rounded-xl border border-neutral-200 px-4 pr-16 text-sm outline-none focus:border-neutral-400" placeholder="30">
        ...
        <div class="pt-2 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
            ...
            <button type="submit" class="inline-flex justify-center items-center gap-2 px-5 py-3 rounded-xl bg-neutral-900 text-white text-sm font-medium hover:bg-neutral-800"><i data-lucide="save" class="w-4 h-4"></i>Simpan Paket</button>
        </div>
    </form>
</div>
</main>
